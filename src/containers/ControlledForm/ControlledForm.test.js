...
describe('ControlledForm tests', () => {
	it('should render component', () => {
		const renderedControlledForm = shallow(<ControlledForm user={{}} location={{}} />)
	...
	it('should have a default user state and a default error message state', () => {
		...
		expect(renderedControlledForm.state('errorMessage')).toEqual(defaultErrorState)
	})

	it.skip('should set state when a signup error is passed in from nextProps', () => {
		const renderedControlledForm = shallow(<ControlledForm user={'error-signup'} location={{}} />)
		const signupErrorMessage = 'That email is already registered. Please use another email or try logging in.'
		renderedControlledForm.update();
		expect(renderedControlledForm.state('errorMessage')).toEqual(signupErrorMessage)
	})
